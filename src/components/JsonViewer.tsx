...
export default function JsonViewer({ json, className = '' }: JsonViewerProps) {
  const highlightJson = (jsonString: string): JSX.Element[] => {
    ...
    const addToken = (text: string, type: 'key' | 'string' | 'number' | 'boolean' | 'null' | 'punctuation' | 'whitespace') => {
      ...
      switch (type) {
        case 'key':
          tokens.push(<span key={key} className="text-blue-600 dark:text-blue-400 font-medium">{text}</span>);
          break;
        case 'string':
        ...
        case 'number':
          ...
          break;
        case 'boolean':
          tokens.push(<span key={key} className="text-orange-600 dark:text-orange-400 font-medium">{text}</span>);
          break;
        case 'null':
          tokens.push(<span key={key} className="text-red-600 dark:text-red-400 font-medium">{text}</span>);
          break;
        case 'punctuation':
          tokens.push(<span key={key} className="text-gray-600 dark:text-gray-400">{text}</span>);
          break;
        case 'whitespace':
  ...
  try {
    ...
    return (
      <pre className={`text-sm font-mono whitespace-pre-wrap break-words ${className}`}>
        {highlighted}
      </pre>
    );
  } catch (error) {
